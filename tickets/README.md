...
|---|---|---|---|
| [BUG-001](BUG-001_wrong-openai-client-class.md) | High | llm_service.py:234, tts_service.py:297 | `AsyncAzureOpenAI` used instead of `AsyncOpenAI` for regular OpenAI provider |
| [BUG-002](BUG-002_blocking-tts-call-in-async.md) | High | tts_service.py:165 | Blocking `.get()` call inside async function freezes event loop |
...
|---|---|---|---|
| [ARCH-001](ARCH-001_missing-vector-db-phase1.md) | High | 1 | Vector DB embedding step missing from upload pipeline — blocks Phase 2 RAG |
...
| ID | Severity | File(s) | Summary |
|---|---|---|---|
| [PERF-001](PERF-001_sequential-tts-generation-too-slow.md) | High | tts_service.py:114 | Sequential TTS for 30-80 turns will exceed the 60s US-01 acceptance criterion |
